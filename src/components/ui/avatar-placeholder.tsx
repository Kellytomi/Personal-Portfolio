...
export const AvatarPlaceholder: React.FC<AvatarPlaceholderProps> = ({
  ...
  const maleAvatar = (
    <svg viewBox="0 0 100 100" className={`w-full h-full ${className}`}>
      ...
      {/* Hair */}
      <path d="M35 25 Q50 15 65 25 L65 35 Q50 20 35 35 Z" fill="#2d3748" />
  ...
  const femaleAvatar = (
    <svg viewBox="0 0 100 100" className={`w-full h-full ${className}`}>
      ...
      {/* Hair - longer style */}
      <path d="M30 25 Q50 10 70 25 L70 45 Q50 25 30 45 Z" fill="#8b4513" />
  ...
  return (
    <div className="relative w-full h-full">{gender === 'male' ? maleAvatar : femaleAvatar}</div>
  );
};
